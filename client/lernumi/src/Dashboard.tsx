...
import type { DiagramSummary, LernumiDiagrams } from './diagrams';

/**
 * The easydraw dashboard, hosted inside a Lernumi attempt.
 *
 * Same layout as `src/app/(app)/dashboard/page.tsx`, with three differences:
 * diagrams come from the Lernumi workspace instead of `/diagrams`, the account
 * menu shows the assessment instead of settings/log out, and a submitted
 * attempt (`readOnly`) hides every mutating action.
 */
...
export default function Dashboard({ diagrams: store }: { diagrams: LernumiDiagrams }) {
  const navigate = useLernumiApp((s) => s.navigate);
  const context = useLernumiApp((s) => s.context);
  ...
  const [renameTarget, setRenameTarget] = useState<DiagramSummary | null>(null);
  ...
  const handleCreateDiagram = async ({ name, type }: { name: string; type: string }) => {
    const created = await store.create({ title: name, type });
    navigate(`/editor/${created.id}`);
  };

  const openDiagram = (d: DiagramSummary) => {
    setMenuFor(null);
    navigate(`/editor/${d.id}`);
  };

  const askRename = (d: DiagramSummary) => {
    setMenuFor(null);
    setRenameTarget(d);
    setRenameOpen(true);
  };
  ...
  const confirmRename = async (name: string) => {
    ...
    if (!d) return;
    await store.rename(d.id, name);
    setDiagrams((prev) => prev.map((x) => (x.id === d.id ? { ...x, title: name } : x)));
  };

  const duplicateDiagram = async (d: DiagramSummary) => {
    setMenuFor(null);
    const created = await store.duplicate(d.id);
    setDiagrams((prev) => [created, ...prev]);
  };

  const askDelete = (d: DiagramSummary) => {
    setMenuFor(null);
    setDeleteTarget(d);
    setDeleteOpen(true);
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setDiagrams(result);
      } catch (error) {
        if (!cancelled) setLoadError(error instanceof Error ? error.message : 'Could not load diagrams.');
      } finally {
    ...
    <div className="min-h-screen bg-panel">
      ...
      <header className="border-b border-line-soft bg-white">
        <div className="flex w-full items-center justify-between px-5 py-4 sm:px-8">
          ...
          <div className="flex items-center gap-4">
            {readOnly ? (
              <span className="flex min-h-11 items-center gap-2 rounded-xl bg-panel px-4 py-2.5 text-sm font-medium text-ink-muted">
                <Lock size={16} />
                Submitted · read-only
              </span>
            ) : (
            ...
            {/* User menu — shows the Lernumi context instead of account settings */}
                ...
                <>
                  ...
                  <div className="absolute right-0 z-20 mt-1 w-72 overflow-hidden rounded-lg border border-line bg-white shadow-lg">
                    <div className="border-b border-line px-3 py-2.5">
                      ...
                      <p className="truncate text-xs text-ink-muted">{context?.role.roleName}</p>
                    </div>
                    {assessment && offering && (
                      <div className="flex items-start gap-2.5 px-3 py-2.5 text-sm text-ink">
                        <BookOpen size={16} className="mt-0.5 flex-shrink-0 text-ink-muted" />
                    ...
                    <p className="border-t border-line px-3 py-2.5 text-xs leading-5 text-ink-muted">
                      Your diagrams are saved into this attempt on Lernumi. Close this tab to return.
                    </p>
                  </div>
                </>
              )}
      ...
      <main className="mx-auto max-w-7xl px-5 py-9 sm:px-8 sm:py-12">
        ...
        <p className="mt-1.5 text-base text-ink-muted">
          {assessment
            ? `Diagrams for ${assessment.name}${offering ? ` · ${offering.subject.code}` : ''}`
            : 'Create and manage your technical diagrams'}
        </p>

        {loading ? (
          <p className="mt-9 text-center text-ink-muted">Loading...</p>
        ) : loadError ? (
          <p className="mt-9 text-center text-danger">{loadError}</p>
        ) : diagrams.length === 0 ? (
          <section className="mt-9 flex min-h-[390px] flex-col items-center justify-center overflow-hidden rounded-3xl border border-line-soft bg-white px-6 py-14 text-center shadow-[0_12px_40px_rgba(44,44,42,0.05)] sm:px-10">
            ...
            <p className="mt-2 max-w-md text-base leading-7 text-ink-muted">
              {readOnly
                ? 'This attempt was submitted without any diagrams.'
              ...
              <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {filtered.map((d) => {
                  ...
                  return (
                    ...
                    >
                      <button type="button" onClick={() => openDiagram(d)} className="block w-full text-left">
