...
export const GROUP_LABEL = 'm-0 text-[0.7rem] font-bold tracking-[0.08em] text-primary-deep';
...
export const STEPPER_BTN =
	'h-[22px] w-[22px] cursor-pointer rounded border-none bg-transparent text-base leading-none text-ink-soft hover:bg-[#e4ebec]';
...
export const SQUARE_BTN = [
	...
	'text-[0.95rem] text-ink-soft transition-colors duration-[120ms] hover:border-[#b6c0c2]',
	'[&.active]:border-primary [&.active]:bg-primary-tint [&.active]:text-primary-deep'
].join(' ');

export const TOGGLE = 'relative inline-block h-5 w-9 flex-shrink-0';
export const TOGGLE_SLIDER = [
	'absolute inset-0 cursor-pointer rounded-full bg-[#b6c0c2] transition-colors duration-150',
	...
	"before:duration-150 before:content-[''] peer-checked:bg-primary peer-checked:before:translate-x-4"
].join(' ');
...
export const SWATCH = [
	...
	'[&.is-white]:border-line [&.active]:shadow-[0_0_0_2px_#126862]'
].join(' ');
...
export const ACTION_BTN_DANGER = [
	'cursor-pointer rounded-md border border-danger bg-white p-2.5 text-[0.85rem] text-danger',
	'transition-colors duration-[120ms] hover:bg-danger-tint'
].join(' ');
...
export const NUM_FIELD_LABEL = 'flex-shrink-0 text-[0.78rem] font-semibold text-primary-deep';
